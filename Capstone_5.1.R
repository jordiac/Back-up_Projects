...
source("./Katz_Back-off_2.0.R")


## ******************  WEEK 4 ****************

input1 <- "When you breathe, I want to be the air for you. I'll be there for you, I'd live and I'd"
input2 <- "Guy at my table's wife got up to go to the bathroom and I asked about dessert and he started telling me about his"
input3 <- "I'd give anything to see arctic monkeys this"
input4 <- "Talking to your mom has the same effect as a hug and helps reduce your"
input5 <- "When you were in Holland you were like 1 inch away from me but you hadn't time to take a"
input6 <- "I'd just like all of these questions answered, a presentation of evidence, and a jury to settle the"
input7 <- "I can't deal with unsymetrical things. I can't even hold an uneven number of bags of groceries in each"
input8 <- "Every inch of you is perfect from the bottom to the"
input9 <- "I'm thankful my childhood was filled with imagination and bruises from playing"
input10 <- "I like how the same people are in almost all of Adam Sandler's"
...
## Bigram : treure els stopwords de la columna de prediccions
## Per la resta s'han de treure les paraules sense significat important
## -->treure "s" al final de les paraules ??

## ex: Every inch of you is perfect from the bottom to the
##  --> every inch perfect bottom

## ex: Talking to your mom has the same effect as a hug and helps reduce your
## -->Talking mom effect hug helps reduce

corpus <- Corpus(VectorSource(input4))
aa <- tm_map(corpus, content_transformer(tolower))
aa <- tm_map(aa, removeWords, stopwords("english"))
Dtext <- data.frame(text = sapply(aa, as.character), stringsAsFactors = FALSE)
text <- unlist(Dtext)
...
## Eliminate plurals
corpus <- singularize(unlist(tokenize(x)))
...
finword <- unlist(strsplit(finword, split=" "))
finword <- finword[-1]
save(finword2, file="./finwords2.RData")
remove(text)
gc()

## *****************  Defining N-grams  *******************
## Bigram
bigram2 <- lapply(ngrams(finword, 2), paste, collapse= " " )
bigram2 <- unlist(bigram2)

save(bigram2, file= "./big_tm2.RData")
remove(bigram2)
gc()

## Trigram
trigram2 <- lapply(ngrams(finword, 3), paste, collapse= " " )
trigram2 <- unlist(trigram2)

save(trigram2, file= "./tri_tm2.RData")
remove(trigram2)
gc()

##QuadGram
quadgram2 <- lapply(ngrams(finword, 4), paste, collapse= " " )
quadgram2 <- unlist(quadgram2)

save(quadgram2, file= "./quad_tm2.RData")
remove(quadgram2)
gc()
